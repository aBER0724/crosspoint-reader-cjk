...
class CrossPointSettings : public PersistableStore<CrossPointSettings> {
 ...
 public:
  enum SLEEP_SCREEN_MODE {
    ...
    CUSTOM = 2,
    COVER = 3,
    // Persisted enum values: keep the fork's historic Blank/Cover custom order.
    BLANK = 4,
    COVER_CUSTOM = 5,
    QUICK_RESUME = 6,
  ...
  // UI activities only support portrait and inverted orientations. Values are
  // persisted by index, so keep the fork's historic 0/1 ordering stable.
  ...
  // Color mode (light/dark). Values are persisted by index.
  ...
  // SD card reader font family name (empty = use built-in fontFamily)
  char sdFontFamilyName[32] = "";
  // SD card UI fallback font family name (empty = use built-in UI fonts)
  ...
  // Fork-specific CJK/display settings. Keep their defaults compatible with
  // existing saved settings, then persist them through SettingsList.h.
  uint8_t uiOrientation = UI_PORTRAIT;
  uint8_t firstLineIndent = 0;
  uint8_t invertImages = 0;
  uint8_t colorMode = LIGHT_MODE;

  // Bluetooth page-turn pairing state is intentionally not exposed as a
  // generic SettingsList entry; the Bluetooth manager owns its workflow.
