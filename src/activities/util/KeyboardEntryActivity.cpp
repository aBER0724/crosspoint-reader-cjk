...
fui::KeyboardLayoutId layoutForLanguage(const Language language) {
  // Fork language set is CJK-filtered (EN/CHS/CHT/JA). European layout
  // enums are not present in generated I18nKeys.h, so always use QwertyEn.
  (void)language;
...
bool KeyboardEntryActivity::cursorPositionFromPoint(const int x, const int y, size_t& position) const {
  ...
  const int inputStartY = screen.y + metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing +
