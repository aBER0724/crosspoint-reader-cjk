...
BaseTheme::HintInsets BaseTheme::getButtonHintInsets(const GfxRenderer& renderer) const {
  const int thickness = UITheme::getInstance().getMetrics().buttonHintsHeight;
  switch (renderer.getOrientation()) {
    case GfxRenderer::Orientation::Portrait:
      return {0, 0, thickness, 0};
    case GfxRenderer::Orientation::PortraitInverted:
      return {thickness, 0, 0, 0};
    case GfxRenderer::Orientation::LandscapeCounterClockwise:
      return {0, thickness, 0, 0};
    case GfxRenderer::Orientation::LandscapeClockwise:
      return {0, 0, 0, thickness};
  }
  return {0, 0, 0, 0};
}
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    if (rowValue != nullptr) {
      ...
      if (!valueText.empty()) {
        ...
        rowTextWidth -= valueTextWidth + minValueGap;
      }
    }
